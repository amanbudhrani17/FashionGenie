...
''' Here we have done WebScraping to gather the initial DataSet with its product details, price and 
    images. We will in future also upgrade the recommendation model and chatbot to recommend based on reviews.
    
    Therefore in future we will also add Reviews in scraping.  
'''
...
class scrape:
    ...
    def _processroot(self, rootlinks):
        '''This method scans the home page to pick profcyt links for all categories'''
    ...
    def _processplp(self, plpurl, categoryid):
        '''Basic category and Product Attributes for each CATEGORY '''

        soup = BeautifulSoup("<html/>", 'html.parser')
        self.logger.info("Started... - {}".format(categoryid))
    ...
    def _getpdp(self, url):
        ''' this method will --> initialize and read all variant information, price and also bullet points'''
        colorvars = []
        sizevars = []
        selectedcolor = ""
        price = ""
        ...
        for var in sizevariants:
            ...
            sizevars.append(
                {"size": varsize, "productid": varcode, "href": "{}{}".format(self.ROOT, varhref), "title": vartitle})
        ...
        proddetails = {"price": price, "mrp": mrp,
                       "color": selectedcolor, "coloroptions": colorvariations, "sizeoptions": sizevars,
